...
```{r}
# subsetting data to just be day time data we are making the bootstrap GAM model with. 
...
for (x in n)
{
  ...
  ## using the night time data to build respiration model/nighttime NEE
  if(x == 1){
  reco_control_co2_sample <- night_control_tower_eosense.data 
  ...
  reco.gam <-
    gam(
      ...
      data = reco_control_co2_sample, #using the nighttime data
      method = "REML",
      na.action=na.omit
     
    )
  #do a nee gam with just the daytime data
  
   
  nee.gam_control_day <-
    gam(
      NEE ~ s(VWC_Control_GF, k = 30) + s(Rg, Tair, VPD, k = 30) ,
      data = control_day_co2_sample,#using the daytime data
      method = "REML",
      na.action=na.omit
  ...
  pred_control_co2 <-
    predict(
      reco.gam, #this will be reco
       #nee.gam_control_day, #NEE of day time data
     ...
     # day_control_tower_eosense.data, #just the daytime data do this and overall tower data
      tower_eo_sense.data, #all control side data, day any night
      type = "link",
      se.fit = FALSE,
      terms = NULL,
      exclude = NULL,
...
#find the mean 
tower_eo_sense.data$pred_GAM_reco <- rowMeans(master_control_pred, na.rm = TRUE)
#day_control_tower_eosense.data$pred_GAM_NEE_day_data<- rowMeans(master_control_pred, na.rm = TRUE)
...
tower_eo_sense.data$reco_pred_stdev <- apply(master_control_pred, 1, sd, na.rm = TRUE)
...
pred_control_co2_NEE <-
  ...
  control_co2_pred_NEE <-
    data.frame(matrix(
      unlist(pred_control_co2_NEE),
      nrow = length(pred_control_co2_NEE),
      byrow = TRUE
    ))
  #have GPP somewhere in loop. should be simple one line of code. 
  # control_co2_pred_NEE <- as.data.frame(t( control_co2_pred_NEE))
  # control_co2_pred_NEE = subset( control_co2_pred_NEE, select = -c(V2))
  #  control_co2_pred_NEE$x <- x  use this if we want to know which run is which.
  if(x == 1){
    master_control_pred <-  control_co2_pred_NEE
  }else{
    master_control_pred <- cbind.data.frame(master_control_pred,
                                             control_co2_pred_NEE)   
  }
  ...
  #assign(paste(" control_co2_pred_NEE",x,sep="_"), control_co2_pred_NEE)  ##this is if we want to see each prediction separately (has standard error)
...
#replace and then do the GPP here. For each 1-30 sample. For reco, replacing all modeled values with observed nighttime values. At this point assuming our observerved uncertainty is zero.
...
#upperlim=mean_df$mean+1.960*stdev_df$stdev/sqrt(length(n)) #zvalue for 95% CI= 1.960
#upperlim_df<- as.data.frame(upperlim)
tower_eo_sense.data$upperCI_NEE <- apply(master_control_pred, 1, quantile, probs = 0.975, na.rm = TRUE)

#find lower lim of CI
#lowerlim=mean_df$mean-1.960*stdev_df$stdev/sqrt(length(n)) #zvalue for 95% CI= 1.960
#lowerlim_df<- as.data.frame(lowerlim)
tower_eo_sense.data$lowerCI_NEE <- apply(master_control_pred, 1, quantile, probs = 0.025, na.rm = TRUE)
```
#plotting predicting all time reco and all time NEE for control side

```{r}
plot(tower_eo_sense.data$pred_GAM_reco)
plot(tower_eo_sense.data$pred_GAM_NEE)

summary(tower_eo_sense.data$pred_GAM_reco)
summary(tower_eo_sense.data$pred_GAM_NEE)
```
...
plot(tower_eo_sense.data$TIMESTAMP,
      tower_eo_sense.data$upperCI_NEE, lty = 'dashed', col = 'red')
...
lines(tower_eo_sense.data$TIMESTAMP,
 ...
 #respiration
 plot(tower_eo_sense.data$TIMESTAMP,
      tower_eo_sense.data$upperCI, lty = 'dashed', col = 'red')
lines(tower_eo_sense.data$TIMESTAMP,
      tower_eo_sense.data$upperCI, lty = 'dashed', col = 'red')
lines(tower_eo_sense.data$TIMESTAMP,
     tower_eo_sense.data$pred_GAM_reco,
     col="black")
 legend(x= "topleft",
         legend = c("95% CI","mean"),
           pch = 16,
