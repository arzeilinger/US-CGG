calc_footprint_FFP_climatology_v2 <-
            ...
            crop = NULL, pulse = NULL) 
  {
    ...
    # last change: 10/04/2017 natascha kljun
    #
    # Copyright (C) 2015,2016,2017 Natascha Kljun
    ...
    ## modified by hchu, holder for fetch max
    fpt_KL15.max<-data.frame(X_max=rep(NA,ts_len),
                             Y_max=rep(NA,ts_len))
    ...
    #--------------------------------------------------------------------
      ...
      for (foot_loop in 1:ts_len) {
        ...
        if (valid[foot_loop]) {
          ...
          ## modified by hchu, holder for fetch max
          fpt_KL15.max$X_max[foot_loop]<-x_2d[which(f_2d==max(f_2d,na.rm=T))[1]]
          fpt_KL15.max$Y_max[foot_loop]<-y_2d[which(f_2d==max(f_2d,na.rm=T))[1]]
      ...
      if (sum(valid) > 0){
        ...
        if (smooth_data) {
          skernel = matrix(c(0.05, 0.10, 0.05, 
                             0.10, 0.40, 0.10, 
                             0.05, 0.10, 0.05), ncol = 3)
          nr_f <- nrow(fclim_2d)
          nc_f <- ncol(fclim_2d)
          ...
          ## edited by Housen, to avoid error
          fclim_2d_pad[which(is.na(fclim_2d_pad))]<-0
          fclim_2d_pad[which(is.nan(fclim_2d_pad))]<-0
